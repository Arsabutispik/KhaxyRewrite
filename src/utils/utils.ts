...
async function bumpLeaderboard(client: KhaxyClient, guildID: Snowflake, lastBump?: User) {
  ...
  const { rows: rows2 } = (await client.pgClient.query("SELECT users, winner FROM bumpleaderboard WHERE id = $1", [
    guildID,
  ])) as {
    rows: {
      users: { userID: Snowflake; bumps: number }[];
      winner: { user: { userID: Snowflake; bumps: number } | null; totalBumps: number };
    }[];
  };
