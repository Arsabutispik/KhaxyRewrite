...
export default async (client: KhaxyClient) => {
    ...
    const { rows } = await client.pgClient.query('SELECT id, days_to_kick, register_channel, member_role, mute_role, language from guilds') as { rows: GuildTypes[] };
    ...
    for (const row of rows) {
        ...
        try {
            ...
            .forEach((member) => {
                ...
                member.send(t("unregistered_member.kick_dm", { guild: guild.name, days: days_to_kick }))
                    .catch(() => {});

                // Kick the member from the guild
                member.kick(t("unregistered_member.initial", { days: days_to_kick }))
                    .catch(() => {});
