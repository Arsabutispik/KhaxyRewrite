...
export default {
  ...
  async execute(interaction) {
    ...
    if(duration && time) {
      ...
      const reply = await modLog({guild: interaction.guild!, user, action: "TIMED_BAN", moderator: interaction.user, reason, duration: dayjs(Date.now() + banDuration), caseID: rows[0].case_id}, client);
      if(reply) {
        interaction.replied ? await interaction.followUp(reply.message) : await interaction.reply(reply.message);
      }
      try {
        await (interaction.client as KhaxyClient).pgClient.query("INSERT INTO punishments (expires, type, user_id, guild_id, staff_id, created_at) VALUES ($1, $2, $3, $4, $5, $6)", [new Date(Date.now() + banDuration), "BAN", user.id, interaction.guild!.id, interaction.user.id, new Date()]);
      } catch (e) {
        logger.error({
      ...
      const reply = await modLog({guild: interaction.guild!, user, action: "BAN", moderator: interaction.user, reason, caseID: rows[0].case_id}, client);
      if(reply) {
        interaction.replied ? await interaction.followUp(reply.message) : await interaction.reply(reply.message);
      }
    }
