...
export default {
    ...
    async execute(member: GuildMember) {
        ...
        const {rows} = await (member.client as KhaxyClient).pgClient.query("SELECT * FROM guilds WHERE id = $1", [member.guild.id]);
        ...
        if (rows[0].goodbye_message && rows[0].goodbye_channel && member.guild.channels.cache.has(rows[0].goodbye_channel)) {
            const goodbye_channel = member.guild.channels.cache.get(rows[0].goodbye_channel)!;
            if (!goodbye_channel.isTextBased()) return;
            if (!goodbye_channel.permissionsFor(member.guild.members.me!)?.has(PermissionsBitField.Flags.SendMessages)) return;
