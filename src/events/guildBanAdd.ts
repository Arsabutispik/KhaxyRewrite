        ...
        try {
            ...
            if (entry.target?.id !== ban.user.id) {
                await modLog({
                    ...
                    user: ban.user,
                    moderator: ban.client.user,
                    reason: (ban.client as KhaxyClient).i18next.getFixedT(rows[0].language)("events:guildBanAdd.executorNotMatch")
                }, ban.client as KhaxyClient);
                return;
            }
            ...
            await modLog({
                ...
                reason: entry.reason || (ban.client as KhaxyClient).i18next.getFixedT(rows[0].language)("events:guildBanAdd.noReason")
            }, ban.client as KhaxyClient);
        } catch (error) {
