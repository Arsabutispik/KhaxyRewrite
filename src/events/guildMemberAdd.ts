...
export default {
    ...
    async execute(member: GuildMember) {
        ...
        const {rows} = await (member.client as KhaxyClient).pgClient.query('SELECT * FROM guilds WHERE id = $1', [member.guild.id]) as {rows: GuildTypes[]};

        // If no guild data is found, exit the function
        if(rows.length === 0) return;

        // Fetch punishment data for the member from the database
        const {rows: punishment_rows} = await (member.client as KhaxyClient).pgClient.query('SELECT * FROM punishments WHERE guild_id = $1 AND user_id = $2 AND type = $3', [member.guild.id, member.id, "mute"]) as {rows: PunishmentsTypes[]};
        ...
        if(rows[0].welcome_message && rows[0].welcome_channel && member.guild.channels.cache.has(rows[0].welcome_channel)) {
            ...
            if(!welcome_channel.permissionsFor(member.guild.members.me!)?.has(PermissionsBitField.Flags.SendMessages)) return;
        ...
        if(rows[0].register_welcome_message && rows[0].register_welcome_channel && member.guild.channels.cache.has(rows[0].register_welcome_channel)) {
            ...
            if(!register_welcome_channel.permissionsFor(member.guild.members.me!)?.has(PermissionsBitField.Flags.SendMessages)) return;
